class HomeController < ApplicationController
  ...
  def user_profile
    respond_to do |format|
      surveys = @currently_signed_in.surveys.order("date_published")
  ...
  def graph_catch_per_effort
    render json: {} if !params[:fishery_id] #
  ...
  def graph_length_frequency
    ...
    from = DateTime.new(Integer(params[:year_from]), Integer(params[:month_from]), 1)
    to = DateTime.new(Integer(params[:year_to]), Integer(params[:month_to]), 1)
    @fishery = Fishery.find(params[:fishery_id]).surveys.where(:date_published => from..to ).each do |s|
