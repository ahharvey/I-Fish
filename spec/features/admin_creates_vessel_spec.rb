...
RSpec.describe "Staff Creates Unloading" do
  ...
  describe "with signed in staff" do
    ...
    it "creates with valid data" do
      ...
      click_link 'Vessels'
      expect(current_path).to eq vessels_path

      expect(page).to have_link 'NEW', href: new_vessel_path
      click_link 'NEW'

      expect(current_path).to eq new_vessel_path


      expect(page).to have_field "Name"
      expect(page).to have_field "Company"
      expect(page).to have_field "Length"
      expect(page).to have_field "GT"

      expect(page).to have_field "AP2HI UVI"
      expect(page).to have_field "ISSF UVI"
      expect(page).to have_field "IMO reg. no."
      expect(page).to have_field "SEAFDEC UVI"
      expect(page).to have_field "MMAF UVI"
      expect(page).to have_field "DKP UVI"
      expect(page).to have_field "SIPI/ SIKPI"
      expect(page).to have_field "SIPI/ SIKPI expiry date"
      expect(page).to have_field "SIUP"

      expect(page).to have_field "No. crew"
      expect(page).to have_field "No. hooks"
      expect(page).to have_field "Captain"
      expect(page).to have_field "Owner"
      expect(page).to have_field "Relationship type"

      expect(page).to have_field "Vessel type"
      expect(page).to have_field "Material type"
      expect(page).to have_field "Gear"
      expect(page).to have_field "Machine"
      expect(page).to have_field "Horsepower"
      expect(page).to have_field "Hold capacity (kg)"
      expect(page).to have_field "Bait capacity (kg)"

      expect(page).to have_field "VMS"
      expect(page).to have_field "Tracker"
      expect(page).to have_field "Shark policy"
      expect(page).to have_field "IUU list"
      expect(page).to have_field "Code of Conduct"
      expect(page).to have_field "Radio"

      expect(page).to have_field "Flag state"
      expect(page).to have_field "Flag state changed"
      expect(page).to have_field "Place of construction"
      expect(page).to have_field "Name changed"
      expect(page).to have_field "Year of Construction"

      expect(page).to have_field "Operational Status"

      expect(page).to have_field "Marked"
      expect(page).to have_field "Member"
      expect(page).to have_field "Certification status"

      fill_in 'Name',  with: 'Vessel name'
      fill_in 'AP2HI UVI',  with: '123456789'

      click_button 'Save'

      expect(page).to have_content 'Vessel was successfully created'
      expect(current_path).to eq vessel_path(Vessel.last)
  ...
  describe "with signed in enumerator" do
    ...
    it "does not allow create" do
      ...
      expect(page).to_not have_link 'NEW', href: new_vessel_path

      visit new_vessel_path

      expect(page).to have_content "You don't have permission to create vessels"
