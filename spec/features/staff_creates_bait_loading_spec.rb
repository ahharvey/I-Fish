...
RSpec.describe "Staff Creates Unloading" do
  ...
  describe "with signed in staff" do
    ...
    let(:bait1)     { create :bait }
    let(:bait2)     { create :bait }
    ...
    it "creates with valid data" do
      ...
      expect(current_path).to eq new_vessel_bait_loading_path(vessel)

      fill_in 'Date',  with: Date.yesterday
      select bait1.code, from: 'Bait'
      select bait2.code, from: 'Secondary bait'
      fill_in 'Quantity', with: 5
      fill_in 'Total price',  with: 5000

      click_button 'Save'


      expect(page).to have_content 'Bait loading report was successfully created.'
      expect(current_path).to eq vessel_path(vessel)
    end
