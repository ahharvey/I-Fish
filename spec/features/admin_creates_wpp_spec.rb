...
RSpec.describe "Staff Creates Unloading" do
  ...
  describe "with signed in staff" do
    ...
    it "creates with valid data" do
      ...
      expect(page).to have_link 'WPPs'
      click_link 'WPPs'
      expect(current_path).to eq wpps_path

      expect(page).to have_link 'NEW', href: new_wpp_path
      click_link 'NEW'

      expect(current_path).to eq new_wpp_path

      expect(page).to have_field "Name"
      fill_in 'Name',  with: 'WPP Name ZZZ'
      click_button 'Save'

      expect(page).to have_content  "SUCCESS! Wpp was successfully created."
      expect(current_path).to eq wpp_path(Wpp.last)
      expect(page).to have_content 'WPP Name ZZZ'
  ...
  describe "with signed in enumerator" do
    ...
    it "does not allow create" do
      ...
      expect(page).to_not have_link 'NEW', href: new_wpp_path
