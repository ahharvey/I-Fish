...
RSpec.describe "Staff Creates Port" do
  ...
  describe "with signed in staff" do
    ...
    it "creates with valid data" do
      ...
      click_link 'Ports'
      expect(current_path).to eq ports_path

      expect(page).to have_link 'NEW', href: new_port_path
      click_link 'NEW'

      expect(current_path).to eq new_port_path

      expect(page).to have_field "Name"
      fill_in 'Name',  with: 'Port Name ZZZ'
      click_button 'Save'

      expect(page).to have_content 'Port was successfully created'
      expect(current_path).to eq port_path(Port.last)
      expect(page).to have_content 'Port Name ZZZ'
