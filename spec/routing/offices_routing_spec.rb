...
RSpec.describe OfficesController, type: :routing do
  ...
  describe :helpers do
    it { expect(offices_path).to eq("/offices") }
    it { expect(new_office_path).to eq("/offices/new") }

    I18n.available_locales.each do |locale|
      it { expect(office_path(1, locale: locale)).to eq("/#{locale}/offices/1") }
      it { expect(edit_office_path(1, locale: locale)).to eq("/#{locale}/offices/1/edit") }
    end
  end
end
