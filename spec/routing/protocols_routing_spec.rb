...
RSpec.describe ProtocolsController, type: :routing do
  ...
  describe :helpers do
    it { expect(protocols_path).to eq("/protocols") }
    it { expect(new_protocol_path).to eq("/protocols/new") }

    I18n.available_locales.each do |locale|
      it { expect(protocol_path(1, locale: locale)).to eq("/#{locale}/protocols/1") }
      it { expect(edit_protocol_path(1, locale: locale)).to eq("/#{locale}/protocols/1/edit") }
    end
  end
end
