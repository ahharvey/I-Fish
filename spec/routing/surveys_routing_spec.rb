...
RSpec.describe SurveysController, type: :routing do
  ...
  describe :helpers do
    it { expect(surveys_path).to eq("/surveys") }
    it { expect(new_survey_path).to eq("/surveys/new") }

    I18n.available_locales.each do |locale|
      it { expect(survey_path(1, locale: locale)).to eq("/#{locale}/surveys/1") }
      it { expect(edit_survey_path(1, locale: locale)).to eq("/#{locale}/surveys/1/edit") }
    end
  end
end
