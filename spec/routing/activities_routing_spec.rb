...
RSpec.describe ActivitiesController, type: :routing do
  ...
  describe :helpers do
    it { expect(activities_path).to eq("/activities") }
    it { expect(new_activity_path).to eq("/activities/new") }

    I18n.available_locales.each do |locale|
      it { expect(activity_path(1, locale: locale)).to eq("/#{locale}/activities/1") }
      it { expect(edit_activity_path(1, locale: locale)).to eq("/#{locale}/activities/1/edit") }
    end
  end
end
