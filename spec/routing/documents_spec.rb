...
RSpec.describe DocumentsController, type: :routing do
  ...
  describe :helpers do
    it { expect(documents_path).to eq("/documents") }
    it { expect(new_document_path).to eq("/documents/new") }

    I18n.available_locales.each do |locale|
      it { expect(document_path(1, locale: locale)).to eq("/#{locale}/documents/1") }
      it { expect(edit_document_path(1, locale: locale)).to eq("/#{locale}/documents/1/edit") }
    end
  end
end
